"""Score the third-seed sweep's 0.10-0.20 bisect (survey/tides pair).

Same bag-delta measurement as score_sweep3.py, pointed at the finer
0.125/0.15/0.175 corpus built by make_sweep3_lowbisect.py — the strengths
seeds 1 and 2 were already bisected at, so seed 3's MinerU onset can be
bracketed at the same resolution as theirs.

Run: uv run python study/sweep/score_sweep3_lowbisect.py
"""
...
def main() -> None:
    truth = json.loads((ROOT / "sweep3_lowbisect_ground_truth.json").read_text())
    truth_toks = {p["page"] - 1: toks(p["text"]) for p in truth["pages"]}
    strengths = {p["page"] - 1: p["ghost_strength"] for p in truth["pages"]}

    engines = {
        "marker": marker_pages(
            ROOT / "marker_out3_lowbisect" / "sweep3_lowbisect" / "sweep3_lowbisect.md", len(truth_toks)
        ),
        "mineru": mineru_pages(
            ROOT / "mineru_out3_lowbisect" / "sweep3_lowbisect" / "hybrid_auto" / "sweep3_lowbisect_content_list.json",
            len(truth_toks),
        ),
    }
    ...
    out = ROOT / "sweep3_lowbisect_results.json"
